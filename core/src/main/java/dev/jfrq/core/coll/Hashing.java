...
final class Hashing {
    ...
    static int freeFor(int capacity) {
     ...
     */
    static boolean mayMove(int slot, int next, int home) {
        boolean homeAfterHole = slot <= next ? slot < home && home <= next : slot < home || home <= next;
