...
public record Snapshot(Path file, long bytes, Instant stop) {
     ...
     */
    public static Snapshot take(FlightRecorderMXBean fr, long recordingId, Window window, Path file) throws IOException {
        long clone = fr.cloneRecording(recordingId, true);
        try {
            Instant stop = stopTime(fr, clone);
    ...
    private static Instant stopTime(FlightRecorderMXBean fr, long clone) throws IOException {
